...
sub read_infoheader() {
  ...
  $bmp->_debug(1,"Image: IndexedColor=$bmp->{IndexedColor} ColorsUsed=$bmp->{ColorsUsed} _colors=$bmp->{_colors}\n");
}
...
sub decolormap {
 ...
 #fatal(sprintf("Color 0x%x not found in orginal colormap\nCurrently the colormap is not updated with new colors\n", $color));
  $index = $bmp->{ColorsUsed}++;
  printf "new colormap[$index]=0x%x\n", $color;
  $bmp->{_colors} = $bmp->{ColorsUsed};
  $bmp->{Index}{rgb}[$index]  = $color;
  $bmp->{Index}{back}{$color} = $index;
  return $index;
}
